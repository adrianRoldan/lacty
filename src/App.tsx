...
export default function App() {
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-svh bg-cream-50">
        ...
        <aside className="hidden lg:flex lg:flex-col lg:w-60 lg:shrink-0 lg:sticky lg:top-0 lg:h-svh bg-white border-r border-gray-200 px-3 py-5">
          ...
          <div className="mt-auto pt-4 border-t border-gray-100">
            <div className="flex items-center justify-between px-3 py-2">
              ...
              <button
                onClick={async () => { await api.logout(); setCurrentUser(null); }}
                className="text-gray-400 hover:text-red-500 p-1.5 rounded-lg touch-manipulation shrink-0"
                aria-label="Salir"
                title="Salir"
              >
                <LogoutIcon />
      ...
      {!showForm && config && (
        <BabyBar babies={babies} activeId={config.id} onSwitch={switchBaby} />
      )}
...
function BabyBar({ babies, activeId, onSwitch }: {
  babies: BabyConfig[];
  activeId: string;
